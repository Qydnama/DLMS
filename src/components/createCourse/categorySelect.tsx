...
interface CategorySelectProps {
  ...
  export const CategorySelect = ({ selected, setSelected }: CategorySelectProps) => {
  const [newTag, setNewTag] = useState<string>("")
  const [expandedCategories, setExpandedCategories] = useState<string[]>([])
  const [searchTerm, setSearchTerm] = useState<string>("")
  const [flattenedTags, setFlattenedTags] = useState<{ id: string; label: string; path: string[] }[]>([])
  ...
  useEffect(() => {
    const flatten = (
      ...
      return categories.flatMap((category) => {
        ...
        const result = [{ id: category.id, label: category.id, path: currentPath }]
  
        if (category.subcategories?.length) {
          return [...result, ...flatten(category.subcategories, currentPath)]
        }
    ...
    return categories
      .map((category): Category | null => {
        ...
        if (category.subcategories && category.subcategories.length > 0) {
          filteredSubcategories = filterCategories(category.subcategories, searchTerm);
        }
      ...
      return (
        <div key={category.id} className="w-full">
          ...
          >
            ...
            {selected.includes(category.id) && <CheckIcon size={14} className="text-goluboy" />}
          </TagsItem>

          {hasSubcategories && isExpanded && (
            <div className="w-full">{renderCategories(category.subcategories!, level + 1)}</div>
          )}
        </div>
      )
    })
      ...
      <TagsContent className="">
        ...
        <TagsList>
          <TagsEmpty>
            ...
            {!newTag.trim() && "No tags found."}
          </TagsEmpty>
          <TagsGroup>{renderCategories(filteredCategories)}</TagsGroup>
        </TagsList>
      </TagsContent>
    </Tags>
  )
